...
Supported states are `INTAKE_READY`, `PLANNING`, `PLANNING_READY`, `ASSETING`, `ASSETS_READY`, `GENERATING`, `PRODUCTION_READY`, `EDITING`, `DRAFT_READY`, `REVIEWING`, `REVIEW_INCOMPLETE`, `REVISION_REQUIRED`, `APPROVED`, `FINAL_RENDERING`, `COMPLETE`, and `BLOCKED`.
...
For live media and review, use the versioned runtime contracts described in `00-media-toolchain.md`. REVIEW_INCOMPLETE is a review capability state, not an artifact defect; preserve the candidate and route to human/capable review without consuming failed-edit cycles. Legacy fixture states remain compatibility proof only.
