...
Confirmed tool repos include Kujo runtime, Kujo Doctor, Agents SDK, AI SDK, Spec, Eval, Scout, Dispatch, MCP, RAG, Watchdog, Scent, Redact, Fence, PatchBrief, ShipCheck, Concord, RunLedger, Muzzle, Howl, ChangeBucket, PackWrite, CaseFile, Lens, Kennel, Kennel Registry, CMS, CRUD API, SSG, AI Chat, Intake, Cinch, Site Kit, Relay, Workcell, Tribunal, Source, StegoCipher Kujo, CMS Experience, Capsule benchmark tooling, Benchmark System prompt kits, Kujo Hyperframes, TotalRecall, Ward, Leash, Kujo Docs, `docs.kujolang.ai`, SiteProbe, SearchBridge, ContentGraph, StoryDesk, Dossier, GalleyPack, BluePencil, AssetWorks, VersionSeal, PressWire, ReaderSignal, Commerce, TruthLens, Kujo Pi, Kujolang MCP, Workcell Studio, Hermes Kujo Discord Agents, Agent City, the `agents.kujolang.ai` static agent library, and other benchmark/showcase repos.

Confirmed Kujo AI provider package repos include Anthropic, OpenAI, Gemini, OpenRouter, xAI, Groq, Together, Fireworks, Mistral, Cerebras, DeepInfra, DeepSeek, Cloudflare AI, Hugging Face, Replicate, Cohere, Bedrock, Azure AI, Vertex AI, NVIDIA NIM, Perplexity, Z.ai, fal.ai, Baseten, and Ollama. Treat them as provider adapter surfaces owned by Integration Engineer, Backend Developer, Security Reviewer, QA Lead, and Release Verifier when explicitly assigned. Offline release gates and package fixtures are directly usable; live provider calls require explicit credentials, model choice, quota/cost boundary, data-retention review, and current provider-doc checks.
...
The 2026-08-29 audit window added or refreshed these agent-facing relationships:

- `anthropic`, `openai`, `gemini`, `openrouter`, `xai`, `groq`, `together`, `fireworks`, `mistral`, `cerebras`, `deepinfra`, `deepseek`, `cloudflare-ai`, `huggingface`, `replicate`, `cohere`, `bedrock`, `azure-ai`, `vertex-ai`, `nvidia-nim`, `perplexity`, `zai`, `fal`, `baseten`, and `ollama`: first-party Kujo provider packages now expose native provider clients and/or AI SDK provider drivers with deterministic offline release gates. Treat them as contract-verified provider adapter work for Integration Engineer, Backend Developer, Security Reviewer, QA Lead, and Release Verifier when explicitly assigned. Do not grant live model access, quota use, provider data retention, pricing claims, or current model-capability claims without explicit live authorization and current provider documentation.
- `kujo-pi`: opt-in Pi integration package exposes trusted-project Kujo tools, configured absolute workflow entrypoints, approval gates, path containment, bounded output, and optional Watchdog/Leash network edges. Treat as an integration/security/tooling review target, not as a replacement for Kujo runtime, provider adapters, workflow engines, MCP authorization, or mobile approval policy.
- `kujolang-mcp`: read-only product-specific MCP catalog server for Kujo projects, skills, workflows, installer profiles, and WebOps capabilities. Treat as a directly verified discovery/catalog integration for Research Analyst, Documentation Writer, and Integration Engineer; public deployment claims still require ingress, Worker, and edge verification.
- `workcell-studio`: WebMCP-native workspace UI for small project creation, file inspection/editing, real Workcell execution, Kujo Eval checks, receipts, patches, manifests, and evidence inspection. Treat as a product/review surface for Frontend Developer, Integration Engineer, QA Lead, Visual QA Agent, Security Reviewer, and Release Verifier when explicitly assigned; do not treat it as a generic chain runtime or approval authority.
- `hermes-kujo-discord-agents`: eight Hermes profile contracts, avatars, installer, read-only knowledge MCP, Discord permission runbooks, and bot-loop limits for a resident Kujo Discord team. Treat as a deployment/profile package owned by Documentation Writer, Product Strategist, Integration Engineer, Security Reviewer, and Release Verifier when explicitly assigned. Do not infer Discord credentials, public bot operation, channel authority, or live community automation.
- `docs.kujolang.ai`: current official docs-site repository with versioned site release policy, SSG build, Site Kit assets, favicon generation, GitHub Pages/Cloudflare deployment docs, and Dispatch 1.2 documentation refresh. Treat as the current public docs site surface for Documentation Writer, Frontend Developer, Visual QA Agent, Product Strategist, and Release Verifier; keep component maturity claims tied to each source repository.
- `kujo`, `kujo-skills`, `kujo-workflows`, `dispatch`, `relay`, `watchdog`, `ai-sdk`, `agents-sdk`, `kennel`, `ssg`, `mcp`, `ai-chat`, `cms`, `spec`, `muzzle`, `packwrite`, `runledger`, and `source`: recent changes refresh existing ownership and verification relevance without broadening role authority. Preserve existing separation between planning, implementation, independent verification, release decision, credential use, and durable evidence capture.

The 2026-09-05 audit window added or refreshed these agent-facing relationships:

- `ability`: portable `kujo.ability/v1` operation contracts with canonical digests, registries, policy/approval/idempotency/audit runtime paths, JSON Schema, SDK previews, an offline pack verifier, and local fixture devkit. Treat as directly verified for Systems Architect, Integration Engineer, Backend Developer, Security Reviewer, QA Lead, and Release Verifier when operation semantics, tool projection, approval binding, or execution receipts are in scope. Do not treat Ability as a network server, global registry, identity provider, authorization system, workflow engine, provider framework, credential store, or production execution substrate.
- `ability-gateway`: Cloudflare Workers control plane and remote MCP transport for the controlled-beta Ability surface at `ability.kujolang.ai`, with GitHub OAuth, tenant-scoped catalog/execution, separate approval flow, idempotency, quotas, D1/KV state, audit history, and local/live certification guidance. Treat as an integration/security/release review target; do not claim broader public-beta or managed-service readiness until live refresh/revocation, negative-token, cross-tenant isolation, approval, audit-export, recovery, privacy, and RPO/RTO gates are complete.
- `paperclip`: host plugin package exposing Scent Context Pack, ChangeBucket/PatchBrief Review Pack, and CaseFile Failure Evidence tools inside Paperclip with pinned component snapshots, path/content guards, bounded child-process execution, and UI/contract/supply-chain verification. Treat as a Paperclip host integration for Integration Engineer, Tooling Developer, Security Reviewer, QA Lead, Code Reviewer, and Context Packager; do not treat it as a replacement for Scent, ChangeBucket, PatchBrief, CaseFile, or Paperclip host policy.
- `email`: Kujo-native email infrastructure foundation with deterministic CLI/API/MCP contracts, diagnostics, DNS planning, queues, reputation, suppression, SMTP/HTTPS services, credential files, receipts, provider-feedback ingestion, and extensive local verification. Treat as a task-specific backend/integration/security/release review target only. The repository explicitly remains not ready for production or Internet exposure because live DNS, public listener, provider enrollment, ClamAV deployment, sustained load, multi-node service adoption, and operator credential boundaries remain incomplete.
- `kujo-workflows` and `kujo-skills`: recent commits refresh workflow/skill catalog state, but no narrow chain skill or workflow contract for routine Ability Gateway, Paperclip, or Email delegation was verified this week. Keep their usage task-specific under existing agents until dedicated contracts appear.

The 2026-09-12 audit window added or refreshed these agent-facing relationships:

- `kennel` and `kennel-registry`: the native installer and publishing workflow now use reviewed Kujo-native registry generation, GitHub Release reconciliation, atomic registry commits, production Pages distribution at `kennel.kujolang.ai`, immutable archive/provenance metadata, and installer protocol review notes. Treat Kennel Registry as directly verified distribution evidence for Dependency Scanner, Tooling Developer, Release Verifier, Documentation Writer, and Security Reviewer when package discovery, install provenance, or registry immutability is in scope. Do not treat it as third-party publishing, account management, a database-backed package service, or permission to rewrite historical package artifacts.
- `kujo` and `kujo-native-package-primitives`: recent branches added or reviewed native package publication primitives such as safe directory symlink cleanup, installation import isolation, process replacement, ownership checks, file locking, and atomic symlink publication. Treat these as Kujo runtime/tooling surfaces for Tooling Developer, Security Reviewer, QA Lead, Release Verifier, and Kennel-related work. Do not claim the currently published stable runtime has unreleased branch-only capabilities until tags, release artifacts, and installer evidence agree.
- `kujo-skills`, `kujo-workflows`, and `kujo-agents` VideoOps: reusable video style and release skills moved into `kujo-skills`, workflow fixtures now resolve production media stages through canonical `kujo-agents/videoops/tools`, and this repository owns the executable VideoOps media, audio QA, and exact-candidate review tools. Existing VideoOps Producer and specialist boundaries remain current; the retired standalone `kujo-videoops` repository remains source provenance only, not a runtime dependency.
- `agent-city`: local visual agent-observation and mission-demo app with real SDK/Dispatch/RAG/MCP/Watchdog integrations, a local Codex subscription provider adapter, browser and mission proof scripts, and explicit release blockers. Treat as a product/review surface for Frontend Developer, Integration Engineer, Visual QA Agent, QA Lead, Security Reviewer, Documentation Writer, and Product Strategist when explicitly assigned. Do not treat it as a chain runtime, universal broker, custom-team importer, production release, or authority over Codex subscription credentials.
- `kujolang-mcp`, `mcp`, `docs.kujolang.ai`, and `kujolang.ai-work`: recent public catalog, installer-profile, release-marker, and documentation refreshes distinguish current runtime release state from website metadata and preserve verified deployment/source provenance. Existing Research Analyst, Documentation Writer, Integration Engineer, Frontend Developer, Visual QA Agent, Product Strategist, and Release Verifier ownership remains current.
- `dispatch`, `agents-sdk`, `rag`, `ai-chat`, `watchdog`, `siteprobe`, `lens`, `fence`, `scent`, `scout`, `redact`, `howl`, `dossier`, `shipcheck`, `casefile`, and `email`: recent hardening commits refreshed evidence boundaries, canonical intake, RAG/docs workflows, local browser/fixture reliability, package release pins, and email readiness proof. Preserve existing role ownership and verification relevance without broadening authority.
- `kujo-payments-review`: a branch-local architecture and fixture evidence packet under a docs-site-shaped repository defines proposed Kujo Payments intent/execution/authorization/receipt contracts, lifecycle transitions, provider evidence limits, and acceptance budgets. Treat it as deferred architecture/security/risk evidence for Systems Architect, Backend Developer, Integration Engineer, Security Reviewer, Risk Officer, QA Lead, Release Verifier, and Product Strategist. Do not treat it as an implemented payments tool, checkout runtime, credential broker, production finance authority, or supported chain capability.
- `astra.robertdevore.com` and `astra2.robertdevore.com`: personal static SiteKit/SSG showcase sites with Cloudflare deployment evidence, WebMCP or Pages notes, SEO audit artifacts, and explicit concept/showcase boundaries. Treat as task-specific website surfaces for WebOps, Frontend Developer, Visual QA Agent, Documentation Writer, and Release Verifier only when assigned; do not add them as Kujo agent capabilities.
